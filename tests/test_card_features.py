...
def test_card_features_use_only_previous_matches() -> None:
    ...
    assert second["home_avg_yellows_5"] == 4.0
    assert second["away_avg_yellows_5"] == 5.0
...
def test_referee_history_excludes_current_match() -> None:
    ...
    assert result.loc[1, "referee_matches_before"] == 1
    assert result.loc[1, "referee_avg_yellows_10"] == 10.0
