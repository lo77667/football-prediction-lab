...
## مقارنة raw وPlatt ضمن cutoff

أضيفت `calibration_compare.py` لملاءمة Platt على صفوف التدريب الواقعة قبل cutoff فقط، ثم تقييمها على صفوف الاختبار الواقعة عند أو بعد cutoff. يرفض العقد صفوف التدريب المستقبلية، ويرفض أي موسم محمي `2526` في التدريب أو الاختبار، ويضع `economic_claim_status: not_assessed`. الاختبار لا يختار المعايرة بناءً على holdout ولا يحول تحسن Brier أو Log-loss إلى ادعاء ربحية.


## حالة CI بعد مقارنة Platt

الـcommit `48c0909` منشور على `main`، ونجحت الفحوص المحلية مع **123 اختبارًا**. فشل `quality-gate` البعيد خلال ثوانٍ مع `steps: []` قبل تنفيذ checkout أو preflight أو الاختبارات، لذلك لا يُنسب الفشل إلى مقارنة المعايرة أو الكود المحلي.


## تقوية مدخلات مقارنة المعايرة

تتحقق مقارنة raw وPlatt الآن قبل الملاءمة من أن جميع الاحتمالات finite وضمن `[0, 1]` وأن targets ثنائية في التدريب والاختبار. يظل cutoff الزمني وحظر `2526` قائمين. الـcommit `faf7d6e` منشور على `main`، ونجحت الفحوص المحلية مع **125 اختبارًا**. فشل CI البعيد مجددًا قبل أي خطوة (`steps: []`) مع سجل runner فارغ، ولا يُنسب ذلك إلى التعديل المحلي.
...
## دورة الحماية الزمنية التالية

أضيف إلى `SourceSelectionPolicy` شرط fail-closed يمنع قبول أي snapshot إذا كان `captured_at < declared_at`. هذا يحول إعلان المصدر إلى التزام سابق فعليًا بالبيانات المقبولة، ويمنع الانتقاء الرجعي لمصدر أو لقطة بعد رؤية حركة السوق. أضيف اختبار regression لهذا الشرط، وارتفع العدد المحلي إلى 130 اختبارًا ناجحًا.

التحسين التالي المقترح هو **profiling لجودة odds snapshots على مستوى المصدر والسوق**: اكتمال التغطية، تكرار اللقطات، فجوات الزمن، وتعارضات الهوية، مع manifest وSHA-256 لكل تقرير. لن يتحول أي مصدر إلى benchmark تجاري ما لم يجتز هذا الملف بوثيقة ترخيص وtimestamps قابلة لإعادة التحقق؛ وستظل نتائج 2526 خارج أي قرار ضبط أو اختيار.

## تنفيذ profiler جودة snapshots
